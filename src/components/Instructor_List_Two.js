...
class Instructor_List_Two extends React.Component {
    constructor(props){
        super(props)
        this.state={
            bio:'none'
    ...
    return(
        ...
        {({loading, error, data}) =>{
           ...
           return data.teachers.map(({theirPhoto,teacherBio,classesTaught, teacherName, id, teacher }) => (
            ...
            <img key={id} src={`${theirPhoto.url}`} className={this.state.bio === id ? 'headshot_select' : 'headshot_turned'} onClick={e =>this.addBio(id)}/>
            </td>
...
export default Instructor_List_Two;
